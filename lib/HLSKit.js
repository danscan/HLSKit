...
HLSKit = function HLSKit(config) {
  ...
  _thisHLSKit.PlaylistSession = function PlaylistSession(id, config) {
     ...
     */
    assert(_.isString(id), 'Required parameter \'id\' must be a string');
  ...
  _thisHLSKit.playlistSessionFromStateObject = function playlistSessionFromStateObject(object) {
     ...
     */
    assert(_.isArray(object.mediaSegments), 'mediaSegments property must be an array');

    assert(_.isObject(object.config), 'config property must be an object');
    assert(_.isNumber(object.config.targetDuration), 'config.targetDuration property must be a number');
    assert(_.isNumber(object.config.windowLength), 'config.windowLength property must be a number');

    assert(_.isObject(object.meta), 'meta property must be an object');
    assert(_.isString(object.meta.id), 'meta.id property must be a string');
    assert(_.isBoolean(object.meta.shouldBeAvailable), 'meta.shouldBeAvailable property must be a boolean');
    assert(_.isBoolean(object.meta.isAvailable), 'meta.isAvailable property must be a boolean');
    assert(_.isBoolean(object.meta.shouldFinish), 'meta.shouldFinish property must be a boolean');
    assert(_.isBoolean(object.meta.isFinished), 'meta.isFinished property must be a boolean');

    /**
     * validate media segment properties for each media segment in the array
     */
    for (i in object.mediaSegments) {
      assert(_.isNumber(object.mediaSegments[i].duration), 'duration property of media segment must be a number');
      assert(_.isNumber(object.mediaSegments[i].timeElapsed), 'timeElapsed property of media segment must be a number');
      assert(_.isNumber(object.mediaSegments[i].mediaSequence), 'mediaSequence property of media segment must be a number');
      assert(_.isNumber(object.mediaSegments[i].discontinuitySequence), 'discontinuitySequence property of media segment must be a number');
    }
  ...
  _thisHLSKit.PlaylistSession.prototype.appendMP4 = function appendMP4(path, options, callback) {
    ...
    async.series([
      function calculateDiscontinuitySequenceAndITSOffset(next) {
         ...
         */
        if (! _.isNumber(options.discontinuitySequence)) {
          options.discontinuitySequence = -1;
        }
        if (! _.isNumber(options.itsoffset)) {
          options.itsoffset = 0;
        }
  ...
  _thisHLSKit.PlaylistSession.prototype._createAVConvCommand = function _createAVConvCommand(path, options) {
     ...
     */
    AVConvCommand = 'avconv -y -loglevel error '
    ...
    for (i in _thisHLSKit._outputVariants) {
       ...
       * add audio options
       */
      variantOutputOptions += '-acodec libfaac '
                            + '-ar '+ _thisHLSKit.config.audioOutputOptions.sampleRate +' '
                            + '-b:a '+ _thisHLSKit.config.audioOutputOptions.bitrate +' ';
  ...
  _thisHLSKit.PlaylistSession.prototype._serializeMasterPlaylists = function _serializeMasterPlaylists(callback) {
    ...
    for (i in _thisHLSKit._outputVariants) {
      ...
      if (variant.config.bitrate && variant.config.frameRate) {

        /**
         * Video variant bandwidth is equal to
         * ((VariantBitrate + AudioBitrate) * VariantFrameRate) + StandardDeviation.
         * StandardDeviation is 0.09
         */
        bandwidth = ((variant.config.bitrate + _thisHLSKit.config.audioOutputOptions.bitrate) * variant.config.frameRate) * 0.09;
      } else {
        bandwidth = 64000;
      }
  ...
  _thisHLSKit.PlaylistSession.prototype._serializeLivePlaylists = function _serializeLivePlaylists(callback) {
    ...
    for (var i = 0; i < liveWindow.length; i++) {
      ...
      thisMediaSegment.duration = (_.isNaN(thisMediaSegment.duration) || thisMediaSegment.duration === NaN || !_.isNumber(thisMediaSegment.duration) ? 0 : thisMediaSegment.duration);
    ...
    /**
     * End live playlist if it should finish
     */
    if (_thisPlaylistSession.meta.shouldFinish) {
      livePlaylist += '#EXT-X-ENDLIST\n';
    }
  ...
  _thisHLSKit.PlaylistSession.prototype._serializeReplayPlaylists = function _serializeReplayPlaylists(callback) {
    ...
    for (var i = 0; i < replayPlaylistSegments.length; i++) {
      ...
      if (_.isNumber(thisMediaSegment.duration)) {
        replayPlaylist += '#EXTINF:'+ thisMediaSegment.duration +',\n'
                        + 'fileSequence'+ thisMediaSegment.mediaSequence +'.ts\n';
      } else {
        debug('Duration of media segment with mediaSequence %s IS NOT a number.', thisMediaSegment.mediaSequence);
      }
