...
HLSKit = function HLSKit(config) {
  ...
  _thisHLSKit.PlaylistSession.prototype._serializeMasterPlaylists = function _serializeMasterPlaylists(callback) {
    ...
    for (i in _thisHLSKit._outputVariants) {
      ...
      if (variant.config.bitrate && variant.config.frameRate) {
         ...
         * StandardDeviation = (VariantBandwidth * 0.01)
         */
        bandwidth = ((variant.config.bitrate + _thisHLSKit.config.audioOutputOptions.bitrate) * variant.config.frameRate);
        bandwidth = bandwidth + (bandwidth * 0.01);
      } else {
        bandwidth = 64000;
      }
  ...
  _thisHLSKit.PlaylistSession.prototype._serializeLivePlaylists = function _serializeLivePlaylists(callback) {
    ...
    for (var i = 0; i < liveWindow.length; i++) {
      ...
      if (_.isNumber(thisMediaSegment.duration) && !_.isNaN(thisMediaSegment.duration) && thisMediaSegment.duration > 1) {
