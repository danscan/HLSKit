  ...
  _thisHLSKit.PlaylistSession.prototype._serializeMasterPlaylists = function _serializeMasterPlaylists(callback) {
    ...
    for (i in _thisHLSKit._outputVariants) {
      ...
      if (variant.config.bitrate && variant.config.frameRate) {
         ...
         * ((VariantBitrate + AudioBitrate) * VariantFrameRate) * 0.0754 [standard dev].
         */
        bandwidth = ((variant.config.bitrate + _thisHLSKit.config.audioOutputOptions.bitrate) * variant.config.frameRate) * 0.0754;
      } else {
        bandwidth = 64000;
      }
